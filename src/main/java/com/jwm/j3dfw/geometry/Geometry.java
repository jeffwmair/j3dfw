...
public class Geometry {
    ...
    public void addChild(Geometry child) {
        log.debug("addChild: adding {} to this:{}", child, this);
        children.add(child);
    }
    ...
    public void getCenter(Vertex point) {
        ...
        if (m == null) {
            if (children == null || children.size() == 0) {
                throw new RuntimeException("Cannot get the center for this (" + this
                        + ") because it has no mesh and has no child geometry items.");
            }
